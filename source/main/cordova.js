...
(function() {
    ...
    define("cordova/pluginloader", function(require, exports, module) {
        ...
        function findCordovaPath() {
            ...
            var term = 'js/jquery-1.8.3.min.js';
    ...
    define("com.coocaaosapi", function(require, exports, module) {
        ...
        CoocaaOSApi.prototype.addUserChanggedListener = function(listener) {
            console.log("cordova addUserChanggedListener");
        ...
        *启动小程序应用
        ...
        CoocaaOSApi.prototype.startAppX3 = function(uri, preload, success, error) {
...
function coocaakeymap(buts, curlink, hover, getVal, setVal, keyDownEvent) {
    ...
    // _this.linkbuttons.unbind('click').bind('click', function(ev) {
    //     _this.linkbuttons.removeClass(_this.hoverClass);
...
var coocaaApp = {
    ...
    getTvSource: function(callback) {
        var _tvsource = "";
        coocaaosapi.getDeviceInfo(function(message) {
            var deviceInfo = message;
            // console.log("deviceinfo==============" + JSON.stringify(deviceInfo));
            var macAddress = message.mac;
            var TVmodel = message.model;
            var TVchip = message.chip;
            var activityId = message.activeid;
            var callbackMsg = {};
            if (message.emmcid == "" || message.emmcid == null) {
                emmcId = "123456";
                deviceInfo["emmcid"] = "123456";
            } else {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.code == 0) {
                        _tvsource = data.data.source;
                    } else {
                        _tvsource = "getTvSourceError";
                    }
                    callbackMsg["tvsource"] = _tvsource;
                    callbackMsg["deviceInfo"] = deviceInfo;
    ...
    haslogin: function(callback) {
        coocaaApp.getTvSource(function(msg) {
            var needQQ = false;
            if (msg.tvsource.indexOf("tencent") != -1) {
                needQQ = true;
            }
            coocaaosapi.hasCoocaaUserLogin(function(message) {
                console.log("haslogin " + message.haslogin);
                var loginstatus = message.haslogin;
                var tencentWay = "";
                var user_flag = "";
                var access_token = "";
                var login_type = "";
                ...
                if (loginstatus == "false") {
                    ...
                    user_flag = 0;
                    access_token = "";
                    callbackMsg["loginstatus"] = loginstatus;
                    callbackMsg["tencentWay"] = tencentWay;
                    callbackMsg["user_flag"] = user_flag;
                    callback(callbackMsg)
                } else {
                    coocaaosapi.getUserInfo(function(message) {
                        ...
                        coocaaosapi.getUserAccessToken(function(message) {
                            ...
                            callbackMsg["tencentWay"] = tencentWay;
                            callbackMsg["user_flag"] = user_flag;
                            callbackMsg["user_info"] = userInfo;
                            callbackMsg["access_token"] = access_token;
                            callbackMsg["login_type"] = login_type;
                            callbackMsg["vuserid"] = vuserid;
                            callback(callbackMsg)
                            // 
                        }, function(error) {
                            console.log(error);
    ...
    judgeJump: function(obj, success, error) {
        console.log("getinfo==="+JSON.stringify(obj))
        var pkgname = obj.packagename || "";
        var needversioncode = obj.versioncode || "-1";
        var missionversioncode = obj.missionversioncode || "-1";
        ...
        coocaaosapi.getAppInfo(a, function(message) {
            if (JSON.parse(message)[pkgname].status == -1) {
                ...
                success({ code: 1001, msg: "应用不存在，请前往下载" });
            } else {
                ...
                if (hasversioncode < needversioncode) {
                    ...
                    success({ code: 1002, msg: "当前版本过低，请进行升级" });
                } else {
                    if (hasversioncode < missionversioncode) {
                        console.log("当前版本可执行跳转，但不符合建议版本要求，建议进行升级");
                        success({ code: 1003, msg: "当前版本可执行跳转，但不符合建议版本要求，建议进行升级" });
                    } else {
                        console.log("当前版本符合要求，可进行后续操作");
                        success({ code: 1000, msg: "当前版本符合要求，可进行后续操作" });
                    }
                }
            }
        }, function(err) {
            console.log("查询应用信息失败");
            error({ code: 1004, msg: "查询应用信息失败" });
        });
    },
    startNormalJump: function(obj,success,error) {
        var pkgname = obj.packagename || "";
        var bywhat = obj.bywhat || "";
        var byvalue = obj.byvalue || "";
        var jumpBgImgUrl = obj.jumpBgImgUrl || "";
        var jumpImgUrl = obj.jumpImgUrl || "";
        var jumpRemindImgUrl = obj.jumpRemindImgUrl || "";
        var countdown = obj.countdown || "";
        var params = obj.params || {};
        var isApkTaskJump = obj.isApkTaskJump || false;
        var taskId = obj.taskId || "";
        var activityId = obj.activityId || "";
        var userKeyId = obj.userKeyId || "";
        var adressIp = obj.adressIp || "";
        var isFinish = obj.isFinish || "false"; //字符串'true'，'false';remainingNumber剩余完成次数，大于0则为false；
        var param1 = "",
            param2 = "",
            param3 = "",
            param4 = "",
        ...
        if (isApkTaskJump) {
            str = JSON.parse(str);
            var external = { "taskId": taskId, "id": activityId, "userKeyId": userKeyId, "countDownTime": countdown, "verify_key": new Date().getTime(), "subTask": "0", "isFinish": isFinish, "jumpBgImgUrl": jumpBgImgUrl, "jumpImgUrl": jumpImgUrl, "jumpRemindImgUrl": jumpRemindImgUrl, "serverUrl": adressIp + "/building/v2/app" };
            var doubleEggs_Active = { "doubleEggs_Active": external };
            str.push(doubleEggs_Active);
            str = JSON.stringify(str);
            console.log(param1+"-----2"+param2+"-----3"+param3+"-----4"+param4+"-----5"+param5+"-----str"+str)
            coocaaosapi.startCommonNormalAction(param1, param2, param3, param4, param5, str, function() {success()}, function() {error()});
        } else {
            console.log(param1+"-----2"+param2+"-----3"+param3+"-----4"+param4+"-----5"+param5+"-----str"+str)
            coocaaosapi.startCommonNormalAction(param1, param2, param3, param4, param5, str, function() {success()}, function() {error()});
        }
    },
    startAppxJump:function(appx_url,success,error){
        coocaaosapi.startAppX2(appx_url, "false", function(){success()}, function(){error()});
        //coocaaosapi.startAppX3(appx_url, "false", function(){success()}, function(){error()});
    }
};
